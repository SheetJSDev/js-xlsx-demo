...
function matchtag(f,g) {return new RegExp('<(?:\\w+:)?'+f+'(?: xml:space="preserve")?(?:[^>]*)>([^\u2603]*)</(?:\\w+:)?'+f+'>',(g||""));}
...
__toBuffer = ___toBuffer = function a_toBuffer(bufs) {
	var x = [];
	for(var i = 0; i != bufs[0].length; ++i) { x = x.concat(bufs[0][i]); }
	return x;
};
if(typeof Buffer !== "undefined") {
	Buffer.prototype.hexlify= function b_hexlify() { return this.toString('hex'); };
	__toBuffer = function b_toBuffer(bufs) { try { return Buffer.concat(bufs[0]); } catch(e) { return ___toBuffer(bufs);} };
}
...
function parse_ws_xml(data, opts, rels) {
	...
	var mtch=data.match(/<(?:\w+:)?sheetData>([^\u2603]*)<\/(?:\w+:)?sheetData>/);
	if(mtch) parse_ws_xml_data(mtch[1], s, opts, refguess);

	/* 18.3.1.48 hyperlinks CT_Hyperlinks */
	if(data.indexOf("</hyperlinks>")!==-1) parse_ws_xml_hlinks(s, data.match(/<hyperlink[^>]*\/>/g), rels);
...
function write_ws_xml_data(ws, opts, idx, wb) {
	...
	for(var R = range.s.r; R <= range.e.r; ++R) {
		...
		for(var C = range.s.c; C <= range.e.c; ++C) {
			...
			ref = cols[C] + rr;
			if(!ws[ref]) continue;
			if((cell = write_ws_xml_cell(ws[ref], ref, ws, opts, idx, wb))) r += cell;
		}
		if(r.length) o[o.length] = (writextag('row', r, {r:rr}));
	}
	return o.join("");
}
...
function encode_col(col) { var s=""; for(++col; col; col=((col-1)/26)|0) s = String.fromCharCode(((col-1)%26) + 65) + s; return s; }
...
function sheet_to_json(sheet, opts){
	...
	range = opts.range !== undefined ? opts.range : sheet["!ref"];
	if(opts.header === 1) header = 1;
	else if(opts.header === "A") header = 2;
	else if(Array.isArray(opts.header)) header = 3;
	switch(typeof range) {
	...
	for(C = r.s.c; C <= r.e.c; ++C) {
		...
		switch(header) {
			...
			case 3: hdr[C] = opts.header[C - r.s.c]; break;
			default:
				if(!val) continue;
	...
	for (R = r.s.r + offset; R <= r.e.r; ++R) {
		...
		for (C = r.s.c; C <= r.e.c; ++C) {
			...
			if(v !== undefined) {
				row[hdr[C]] = opts.raw ? v : format_cell(val,v);
				isempty = false;
			}
		}
...
function sheet_to_row_object_array(sheet, opts) { if(opts == null) opts = {}; return sheet_to_json(sheet, opts); }

function sheet_to_csv(sheet, opts) {
	var out = "", txt = "", qreg = /"/g;
	opts = opts || {};
	if(!isval(sheet) || !isval(sheet["!ref"])) return "";
	var r = safe_decode_range(sheet["!ref"]);
	var FS = opts.FS !== undefined ? opts.FS : ",", fs = FS.charCodeAt(0);
	var RS = opts.RS !== undefined ? opts.RS : "\n", rs = RS.charCodeAt(0);
	var row = "", rr = "", cols = [];
	var i = 0, cc = 0, val;
	var R = 0, C = 0;
